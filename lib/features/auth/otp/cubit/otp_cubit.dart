...
class OtpCubit extends Cubit<OtpState> {
  ...
  Future<void> verifyOtp(String otpCode) async {
    if (verificationId == null) {
      emit(OtpVerificationFailure("Verification process not initiated correctly."));
      return;
    }
    if (otpCode.length != 6) { 
    ...
    try {
      ...
      log("Simulating OTP Verification for code: $otpCode with verificationId: $verificationId");
      ...
      emit(OtpVerificationFailure("OTP verification failed. Please try again.")); // Needs localization
    }
  }
}
